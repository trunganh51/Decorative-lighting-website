...
--
-- Host: 127.0.0.1:3386
-- Generation Time: Nov 22, 2025 at 02:04 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `recalc_order_total` (IN `p_order_id` INT)   BEGIN
    ...
    SELECT shipping_fee, tax, coupon_code
      INTO v_shipping, v_tax, v_coupon_code
    FROM orders
    ...
    IF v_coupon_code IS NOT NULL AND v_coupon_code <> '' THEN
        SELECT discount_type, value, min_subtotal, active
          INTO v_type, v_value, v_min_sub, v_active
        FROM coupons
...
(11, 14, 2, 'Fourteen', '211312', 15, 'aaa', 'standard', 30000.00, NULL, NULL, NULL, '', 67000.00, 0.00, NULL, 767000.00, 'Chờ duyệt', 'pending', NULL, '2025-11-16 10:18:47', '2025-11-16 10:18:47'),
(12, 21, 2, 'A', '1231', 15, '123', 'standard', 30000.00, NULL, NULL, NULL, '', 14500.00, 0.00, NULL, 189500.00, 'Chờ duyệt', 'pending', NULL, '2025-11-16 12:16:59', '2025-11-16 12:16:59');
...
(32, 5, 'Dây điện lõi đồng Cadivi 2x1.5', 'Dây đôi cách điện PVC, dài 100m', 670000.00, NULL, 176, 111, 'images/day-dien-cadivi-2x1.5.jpg'),
(33, 5, 'Bộ chao đèn treo trần', 'Phụ kiện trang trí, chao nhôm trắng', 145000.00, NULL, 87, 36, 'images/chao-den-treo-tran.jpg'),
...
(21, 'tranh', 'lopa59733@gmail.com', '12', 'user', '2025-11-11 12:21:45', NULL, NULL);
...
--
ALTER TABLE `inventory_movements`
  MODIFY `inventory_movement_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;
...
--
ALTER TABLE `remember_tokens`
  MODIFY `remember_token_id` int(11) NOT NULL AUTO_INCREMENT;
